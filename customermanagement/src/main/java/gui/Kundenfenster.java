...
public class Kundenfenster extends JFrame {
		...
		try {
			...
			if (einKunde != null) {
				...
				JOptionPane.showMessageDialog(Kundenfenster.this,
						"Kunde mit Nr. " + nummer + " konnte nicht gefunden werden.", "Mitteilung",
						JOptionPane.OK_OPTION);
				zuruecksetzen();
			}
			return;
		} catch (Exception e) {
			JOptionPane.showMessageDialog(Kundenfenster.this, "Fehlerhafte Eingabe!", "Mitteilung",
					JOptionPane.OK_OPTION);
			einKunde = null;
			zuruecksetzen();
		}
	...
	/*
	 * Alle Daten aus den Eingabeelementen uebernehmen. Gibt true zurueck, falls
	 * erfolgreich, ansonsten false.
	 */
	private boolean schreibeKunde() {
		...
		if (nummer.length() == 0 || name.length() == 0) {
			JOptionPane.showMessageDialog(this, "Es sind Mussfelder nicht ausgefuellt!", "Mitteilung",
					JOptionPane.OK_OPTION);
			return false;
		}
		try {
