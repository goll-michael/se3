...
public class LoginModulKV implements LoginModule 
{
	...
	public boolean commit() throws LoginException 
	{
		...
		{
			if(benutzername.equals(BENUTZER1)) //Benutzer 1 ist Kundenbearbeiter
			{
				if (!benutzer.getPrincipals().contains(Rolle.KUNDEN_BEARBEITER_ROLLE))
					benutzer.getPrincipals().add(Rolle.KUNDEN_BEARBEITER_ROLLE);
			}
			else if(benutzername.equals(BENUTZER2)) //Benutzer 2 ist 'nur' Kunden-Rechercheur
			{
				if (!benutzer.getPrincipals().contains(Rolle.KUNDEN_RECHERCHEUR_ROLLE))
					benutzer.getPrincipals().add(Rolle.KUNDEN_RECHERCHEUR_ROLLE);
			}
